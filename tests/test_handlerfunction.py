...
def test_callFirstLayerValue():
    resVal = yamlHandlers._parsing_yamlFile_omega(workflow_path="tests/", yamlfilename="dummy_config.yaml")
...
def test_callParsingFunction():
    test_input = yamlHandlers._parsing_yamlFile(workflow_path="tests/", yamlfilename="dummy_config.yaml")
    resVal = yamlHandlers._reform_yamlFile("tests/", test_input, "dummy_created")
    with open("tests/dummy_config.yaml", "r") as testFile:
